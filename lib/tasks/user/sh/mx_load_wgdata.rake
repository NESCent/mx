...
# 
  ...
  def generate_person(row = nil)
    if !@people.has_key?(row["entry email"])
        @login = row["entry email"].split('@')
        @person = Person.new(:first_name => login[0],
        :last_name => login[0],
        :login => login[0],
        :password => "nescent!",
  ...
  def generate_taxons(row = nil, taxonrootid = nil)
    # Higher taxonomic group, Order, Family, Genus, species
      ...
      if !@taxons5.has_key?(row[4])
        ...
        taxon = TaxonName.create_new(:taxon_name => {:name => format_species(row[4]), :parent_id => parentid, :iczn_group => iczn_group, :creator_id => @people[@login], :updator_id => @people[@login]}, :person => @person)
        taxon.save!
    ...
    #puts "genref header: "+header
      ...
      if !refstr.nil?
        ...
        if !@refs.has_key?(ref_full) && !ref_full.nil?
          ref = Ref.new(:author => ref_author, :year => ref_year, :full_citation => ref_full, :creator_id => @people[@login], :updator_id => @people[@login])
          if ref.save
  ...
  def generate_otus(row)
    @otu = Otu.new(:name => '', :taxon_name_id => @otuinfo["taxonid"], :notes => row["notes, comments"], :creator_id => @people[@login], :updator_id => @people[@login])
